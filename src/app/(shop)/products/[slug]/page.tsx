...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'website',
      images: product.images[0] ? [{ url: product.images[0].assetId }] : undefined,
    },
  };
}
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const lowestPrice = Math.min(...product.variants.map((v) => v.pricePaise), product.basePricePaise);
  ...
  const productSchema = {
    ...
    image: product.images.map((image) => `${clientEnv.NEXT_PUBLIC_APP_URL}${image.assetId}`),
    offers: {
      '@type': 'AggregateOffer',
      priceCurrency: 'INR',
    ...
    <div className="container-page py-8 md:py-12">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-2 lg:gap-14">
        ...
        <div>
          ...
          {product.summary ? (
            <p className="text-muted-foreground mt-3 text-balance">{product.summary}</p>
          ) : null}
          ...
          <dl className="border-border mt-8 grid grid-cols-2 gap-4 border-t pt-8 text-sm">
            <div>
              <dt className="text-muted-foreground text-xs tracking-wide uppercase">Price range</dt>
            ...
            <div>
              <dt className="text-muted-foreground text-xs tracking-wide uppercase">Warranty</dt>
